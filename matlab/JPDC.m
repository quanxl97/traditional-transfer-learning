function [acc] = JPDC(Xs,Xt,Ys,Yt,options)
    % input Xs: n * m
   ...
   %% iteration
   for i = 1 : T
       [Z,A] = JPDC_core(Xs,Xt,Ys,Yt0,Sw,Sb,options);
       % normalization
...
function [Z,A] = JPDC_core(Xs,Xt,Ys,Yt0,Sw,Sb,options)
   kernel_type = options.kernel_type;
   dim         = options.dim;
   gamma       = options.gamma;
   lambda      = options.lambda;
...
%        [A,~] = eigs(Ms+mu*Sw,lambda*Md,dim,'SM');
        ...
        % 1e-*eye(m)
        Smin = Ms-mu*Md+lambda*eye(m); % +0.1*Sw+0.1*eye(m);  %  
        Smax = X*H*X';  % 0.1*Md+0.1*Sb;  % 
